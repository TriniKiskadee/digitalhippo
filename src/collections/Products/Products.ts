...
const syncUser: AfterChangeHook<Product> = async ({req, doc}) => {
    ...
    if(fullUser && typeof fullUser === "object"){
        ...
        const allIds = [
            ...(products?.map((product) => typeof product === "object"
                ? product.id
                : product) || []),
        ]
...
export const Products: CollectionConfig = {
            ...
            async (args) => {
                if(args.operation === "create") {
                    ...
                    const createProduct = await stripe.products.create({
                        ...
                        default_price_data: {
                            currency: "USD",
                            unit_amount: Math.round(data.price * 100),  // price expressed in cents. Why??? No idea
                        },
                    })
    ...
    fields: [
        ...
        },
        {
            ...
            hasMany: false, // if product can have multiple files (eg. icon set), set to true
            required: true,
        },
        {
